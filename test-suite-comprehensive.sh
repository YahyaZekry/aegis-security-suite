#!/bin/bash
# Garuda Security Suite Comprehensive Test Runner
...
run_integration_tests() {
    ...
    if [ -f "$SCRIPT_DIR/integration-tests/test-security-suite-integration.sh" ]; then
        bash "$SCRIPT_DIR/integration-tests/test-security-suite-integration.sh" 2>&1 | tee -a "$results_file"
        local test_exit=${PIPESTATUS[0]}
        if [ $test_exit -eq 0 ]; then
...
<html>
<head>
    <title>Garuda Security Suite Comprehensive Test Report</title>
    <style>
...
<body>
    <div class="header">
        <h1>🛡️ Garuda Security Suite Comprehensive Test Report</h1>
        <p>Generated: $(date)</p>
    ...
    <div class="summary">
        <h2>📊 Test Summary</h2>
        <p>This report contains the results of comprehensive testing for all Phase 2 components of the Garuda Security Suite, including behavioral analysis, web dashboard, threat intelligence, and incident response systems.</p>
...
# Main execution
main() {
    log_header "Garuda Security Suite Comprehensive Test Runner"
    ...
    cat > "$REPORT_FILE" << EOF
=== Garuda Security Suite Comprehensive Test Report ===
Generated: $(date)
...
case "${1:-}" in
    --help|-h)
        echo "Garuda Security Suite Comprehensive Test Runner"
        echo ""
