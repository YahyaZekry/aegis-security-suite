#!/bin/bash
# Garuda Security Suite Service Management Script
...
declare -A SERVICES=(
    ...
    ["web-dashboard"]="garuda-dashboard"
)
...
is_service_active() {
    ...
    case "$service_name" in
        "web-dashboard")
            ...
            local pid_file="/tmp/garuda-dashboard.pid"
            if [ -f "$pid_file" ]; then
...
run_test() {
    ...
    local test_script="$SECURITY_SUITE_HOME/test-suite-comprehensive.sh"
...
For more information, see the documentation at:
  https://github.com/YahyaZekry/garuda-security-suite/docs/
EOF
}
